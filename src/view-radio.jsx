...
function RadioSettings({
  ...
  const changeTtsDraft = (patch) => setTtsDraft((current) => normalizeTtsConfig({ ...current, ...patch }));

  const testVoice = async () => {
    if (ttsDraft.provider !== 'browser' && !ttsProviderKey(ttsDraft)) {
      setMessage(copy('voiceKeyRequired'));
      return;
    }
    setBusy(true); setMessage(copy('testingVoice'));
    try {
      await onTestTts(ttsDraft, copy('voiceSample'));
      setMessage(copy('voiceTested'));
    } catch (error) { setMessage(error.message); }
    finally { setBusy(false); }
  };
  ...
  const clearVoiceKey = () => {
    ...
    setMessage(copy('voiceKeyCleared'));
  };
    ...
    <div className="auth-overlay" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="auth-modal radio-settings-modal">
        ...
        <div className="radio-voice-section">
          ...
          <div className="radio-setting-actions">
            <button className="btn" type="button" onClick={testVoice} disabled={busy}>{copy('testVoice')}</button>
            {ttsDraft.provider !== 'browser' && <button className="btn" type="button" onClick={clearVoiceKey}
              disabled={busy || !ttsProviderKey(ttsDraft)}>{copy('clearVoiceKey')}</button>}
          </div>
        </div>
